// Source: https://github.com/sdepold/jquery-rss
...
(function ($) {
  ...
  RSS.prototype.getTokenMap = function (entry) {
    ...
    for(var i in entry.categories) {
      if(entry.categories[i].name !== "Job") {
        categoryName = entry.categories[i].name;
      }
    }
    return $.extend({
      ...
      bodyStrip: (function (entry) {
        var result = entry.content
          $( "a" ).remove( ":contains('Read')" );
        return result;
      })(entry),

      // this is an arbitrary trimming of the junk from Jobvite; better would be a stripping of tags, more work needed. -AW
      bodyTrim: (function (entry) {
        console.log(entry.content.length);
        //console.log(entry.link);
        var result = entry.content.slice(0, - 139);
        return result;
      })(entry),
